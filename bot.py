...
# Admin uchun tayyor kategoriyalar klaviaturasi
def admin_category_kb():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="Ceramika"), KeyboardButton(text="Shisha")],
        [KeyboardButton(text="Toshli")],
        [KeyboardButton(text="❌ Bekor qilish")]
    ], resize_keyboard=True)


# ── DINAMIK TUGMALAR (MIJOZ UCHUN) ──
def get_categories_reply_kb(kafels):
    categories = sorted(list(set([str(k['category']).strip() for k in kafels if k.get('category')])))
...
def get_product_control_kb(category: str, current_page: int, total_pages: int):
    builder = InlineKeyboardBuilder()
    for p in range(total_pages):
        btn_text = f"[{p + 1}]" if p == current_page else str(p + 1)
        builder.button(
            text=btn_text, 
            callback_data=ProductPagination(category=category, page=p).pack()
        )
    builder.adjust(5)
    builder.row(
        types.InlineKeyboardButton(
            text=f"📄 {current_page + 1}/{total_pages} (Tavsif)", 
            callback_data=f"kafel_info:{category}:{current_page}"
        )
    )
...
# ── MIJOZ KATEGORIYALARNI KO'RISHI ──
@dp.message(F.text == "🏪 Kafellar ro'yxati")
async def client_show_categories(message: types.Message, state: FSMContext):
    kafels = await db.db_get_kafels()
    if not kafels:
        await message.answer("📭 Hozircha do'konda kafellar mavjud emas.")
        return
    await state.set_state(ClientFilter.select_category)
    await message.answer("🗂 <b>Kafel kategoriyasini tanlang:</b>", parse_mode="HTML", reply_markup=get_categories_reply_kb(kafels))

@dp.message(ClientFilter.select_category, F.text.startswith("📁 "))
async def client_finalize_category(message: types.Message, state: FSMContext):
    category_name = message.text.replace("📁 ", "").strip()
    await state.clear()
    await message.answer("🔄 Kafellar yuklanmoqda...", reply_markup=mijoz_kb())
    
    filtered = await db.db_get_kafels_by_category(category_name)
    if not filtered:
        await message.answer("📭 Ushbu kategoriyada kafellar topilmadi.")
        return
    await send_kafel_page(message, filtered, category_name, 0)

@dp.message(ClientFilter.select_category, F.text == "⬅️ Orqaga")
...
# ══════════════════════════════════════
#   PAGINATION BO'LIMI (KATEGORIYA BO'YICHA)
# ══════════════════════════════════════
...
async def process_pagination(callback_query: types.CallbackQuery, callback_data: ProductPagination):
    filtered = await db.db_get_kafels_by_category(callback_data.category)
    await callback_query.answer()
    try: await callback_query.message.delete()
    except: pass
    await send_kafel_page(callback_query.message, filtered, callback_data.category, callback_data.page)

@dp.callback_query(F.data.startswith("kafel_info:"))
async def process_kafel_description(callback_query: types.CallbackQuery):
    _, category, page_str = callback_query.data.split(":")
    page = int(page_str)
    filtered = await db.db_get_kafels_by_category(category)
    await callback_query.answer()
...
async def send_kafel_page(message: types.Message, kafels_list, category: str, page: int):
    ...
    kb = get_product_control_kb(category, page, total_pages)
...
async def add_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("2️⃣ <b>Kategoriyasini</b> tanlang yoki yozing:", parse_mode="HTML", reply_markup=admin_category_kb())
    await state.set_state(AddKafel.category)

@dp.message(AddKafel.category)
async def add_category(message: types.Message, state: FSMContext):
    if message.text == "❌ Bekor qilish": await state.clear(); await message.answer("Bekor.", reply_markup=admin_kb()); return
    await state.update_data(category=message.text.strip())
    await message.answer("3️⃣ <b>Narxini</b> kiriting (so'mda, faqat raqam):", parse_mode="HTML", reply_markup=cancel_kb())
...
async def add_price(message: types.Message, state: FSMContext):
    ...
    await message.answer("4️⃣ <b>Tavsif</b> kiriting:", parse_mode="HTML", reply_markup=skip_kb())
...
async def add_desc(message: types.Message, state: FSMContext):
    ...
    await message.answer("5️⃣ <b>Rasm</b> yuboring:", parse_mode="HTML", reply_markup=skip_kb())
...
async def _finish_add(message: types.Message, state: FSMContext):
    data = await state.get_data()
    kafel = await db.db_add_kafel(data["name"], data["category"], data["price"], data.get("description", ""), data.get("photo"))
    await state.clear()
